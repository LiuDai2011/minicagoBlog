...
2) 递归解决左半边子问题 $calc(l, mid-1) $ (注意不要包含mid)
3) 此时已经计算出 $[l,mid)$ 的 $f$ 值，用其计算出 $opt_mid(r)$。
...
void calc(int l, int r) {
  ...
  if (l < mid) calc(l, mid - 1);

  //计算opt_mid(r)
  for (int j = l; j < mid; ++j) check(j, r);
  
  if (mid < r) calc(mid, r);
  // 这边mid == r 不需要计算，因为 mid == r 时，上一句话计算出的opt_mid(r)已经是最优解了
}
...
由于 $opt(t)$ 是最优决策点， 有 $ \forall t < opt(x)， f(t, x) < f(opt(x), x) $
