...
函数 $w(l,r)$ 满足四边形不等式，当且仅当$f_i$为$w(i,j)$的最大值（最小值），$\Delta_l \Delta_r w(l,r)$ 非正（或非负）。直观上，满足四边形不等式的函数通常意味着，区间向两侧扩大——即左端点向左移动和右端点向右移动——具有某种协同效应（抑制效应），即同时发生会比单个发生收益更大（更小）。
...
环形问题考虑分别枚举第一个石子是否选取的情况，下面仅考虑链上问题。
